...
###################################################################################################
    ...
    if not content or not title or not date:
        print(generate_error_page("Request lacks body, or query string lacks title or date"))
    else:
        file_name = date + "_" + title + ".html"
        blog = generate_blog_page(title, date, content)
        write_html_to_file(blog, file_name, BLOG_DIR)
        print("Generated article with the URL: localhost/blog/" + file_name)
###################################################################################################
print("</body>")
print("</html>")
